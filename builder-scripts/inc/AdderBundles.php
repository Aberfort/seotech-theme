...
class AdderBundles {
	...
	private function find_chunks_path() {
		...
		if ( $buffer ) {
			...
			wp_die( 'Не удалось считать файл seotech/builder-scripts/stats/stats.json, проверь сгенерировался ли он и есть ли у папки права записи, если вы используете кубернетись то внучную измените stats.json что-то в нем чтоб он пересохранился' );
		}
	...
	private function create_links( $keyPage, $type ): string {
		...
		foreach ( $allFiles as $element ) {
			...
			switch ( $type ) {
				case 'css':
					$links .= '&lt;link rel="preload stylesheet prefetch" as="style" href="' . get_template_directory_uri() . '/bundles/' . $element . '"&gt;';
					break;
				case'js':
					$links .= '&lt;script type="text/javascript" src="' . get_template_directory_uri() . '/bundles/' . $element . '"&gt;&lt;/script&gt;';
					break;
				default:
					wp_die( 'Неверно указан тип необходимых данных в методе create_links' );
			}
	...
	private function get_page_keys( $idCurrentPage ): array {
		...
		foreach ( $array_condition as $keyPage => $item ) {
			...
			if ( in_array( - 1, $item ) || in_array( true, $item ) ) {
				if ( in_array( 1, $item ) ) {
					$result[] = $keyPage;
				} elseif ( in_array( - 1, $item ) && ! in_array( false, $item ) ) {
					$result[] = $keyPage;
				}
			} elseif ( in_array( $idCurrentPage, $item ) && $idCurrentPage ) {
				$result[] = $keyPage;
			}
		}
	...
	public function print_source( $type, $idCurrentPage ) {
		...
		if ( $source ) {
			...
			wp_die( 'Необходимо перезапустить Webpack либо скрипты или стили данной страницы не настроенны (не удалось проверить оптимизацию)' );
		}
	}

	/**
	 * собираем файлы критических стилей
	 */
	public function get_critical_style( $fileID ) {
		$criticalCssFiles = $this->get_source_by_type( $fileID, 'css' );

		if ( $criticalCssFiles ) {
			return $criticalCssFiles;
		} else {
			return '';
		}
	}
	...
	/**
	 * сортируем файлы критических стилей
	 */
	public function sort_critical_style( $pageID ) {
		$pageKeys        = $this->get_page_keys( $pageID );
		$array           = array();
		$criticalSources = array();
		foreach ( $pageKeys as $pageKey ) {
			$pageKeyOne = strripos( $pageKey, 'singleCritical' );
			$chunks     = $this->find_chunks_path()->assetsByChunkName;
			if ( $pageKeyOne !== false ) {
				foreach ( $chunks as $pageKeyChunk => $pageSources ) {
					if ( strripos( $pageKeyChunk, $pageKey ) || $pageKeyChunk == $pageKey ) {

						if ( is_array( $pageSources ) ) {
							$criticalSources = array_merge( $criticalSources, $pageSources );
						} else {
							$criticalSources[] = $pageSources;
						}
					}
				}
			}
		...
		foreach ( $criticalSources as $fileName ) {
		...
		if ( ! count( $array ) ) {
			$array = $this->get_critical_style( 'critical' );
			foreach ( $array as $key => $value ) {
				if ( str_starts_with( $value, "singleCritical" ) ) {
					unset( $array[ $key ] );
				}
			}
		...
		if ( $array ) {
			...
			wp_die( 'Необходимо перезапустить Webpack либо скрипты или стили данной страницы не настроенны (не сортируются критические стили)' );
		}
	}

	/**
	 * получаем содержимое файлов критических стилей
	 */
	public function get_critical_paths( $pageID ) {
		$criticalCss      = '';
		$criticalCssFiles = $this->sort_critical_style( $pageID );

		$dir = dirname( __DIR__, 2 ) . "/bundles";
		if ( count( $criticalCssFiles ) ) {
			foreach ( $criticalCssFiles as $fileName ) {
				$criticalCss .= file_get_contents( $dir . '/' . $fileName );
			}
		}
		if ( $criticalCss ) {
			return $criticalCss;
		} else {
			wp_die( 'Необходимо перезапустить Webpack либо скрипты или стили данной страницы не настроенны (не удалось получить критические стили)' );
		}
	}

	/**
	 * выводит критические скрипты в head
	 */
	public function print_critical_style( $pageID ) {
		$criticalCss = $this->get_critical_paths( $pageID );

		if ( $criticalCss ) { ?>
            <style id="critical"><?php
                echo $criticalCss ?>
            </style><?php
		} else {
			wp_die( 'Необходимо перезапустить Webpack либо скрипты или стили данной страницы не настроенны (не удалось вывести критические стили)' );
		}
	}
